...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen text-black font-sans relative">
      ...
      <div className="brand-mini" style={{ position: 'fixed', top: '12px', left: '16px', fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: '0.9rem', color: '#0f172a', opacity: 0.9 }}>
        AI(ttention)
      </div>

      <div className="mx-auto max-w-lg px-4 sm:px-6 lg:px-8 pt-20 pb-16">
        <div className="text-center mb-8">
          <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">Teacher Sign In</h1>
        </div>

        <div className="relative rounded-2xl border border-white/50 bg-white/70 backdrop-blur-xl shadow-glow p-6 sm:p-8">
          ...
          <input
            ...
            autoComplete="email"
            className="w-full rounded-lg border border-slate-300/80 bg-white/80 backdrop-blur px-4 py-3 shadow-sm focus:outline-none focus:ring-2 focus:ring-emerald-300 focus:border-emerald-300"
          />

          <button
            id="sendCodeBtn"
            type="button"
            className="btn btn-primary glow w-full mt-4 justify-center text-base"
          >
            Send OTP
          </button>
          ...
          <button id="signOutBtn" type="button" className="btn btn-muted w-full mt-4 hidden justify-center">
            Sign Out
          </button>

          <div id="msg" aria-live="polite" className="text-sm text-slate-700 mt-4" />
          <div id="error" aria-live="assertive" className="text-sm text-rose-700 mt-2" />
          <div id="ok" aria-live="polite" className="text-sm text-emerald-700 mt-2" />
        </div>
      </div>
    </div>
  );
