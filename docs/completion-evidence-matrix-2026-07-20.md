...
| Supported production runtime | Node 24 selected in `package.json`/`.nvmrc`; DigitalOcean build used Node 24.15.0; build, smoke, 70 tests, and 25-group simulation pass | Proven locally and in deployment build |
...
| Supabase dashboard Auth/anonymous/CAPTCHA/private Realtime/new keys/ES256 configuration | New keys, OTP, anonymous Auth, and private Realtime work in production; production JWKS still returns zero asymmetric keys; anonymous CAPTCHA is not yet integrated | Partially complete; add CAPTCHA client flow and migrate/rotate ES256 |
...
then disable the legacy API keys before revoking the legacy JWT secret. CAPTCHA integration remains
required before treating anonymous sign-in as hardened against automated account creation.
