...
| Supabase-managed ES256 with no application JWT signing key | Production current key is managed ECC P-256; a freshly issued anonymous token reported `alg: ES256`, matched the public JWKS `kid`, exposed no private key material, and its synthetic user was deleted; a fresh teacher OTP cookie and the complete live production flow passed after rotation | Proven in source and production; the previous HS256 key remains verification-only during the required overlap |
