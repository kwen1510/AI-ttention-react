...
| Supabase dashboard Auth/anonymous/CAPTCHA/private Realtime/new keys/ES256 configuration | New keys, OTP, anonymous Auth, and private Realtime work in production; the client now conditionally passes a Turnstile token directly to `signInAnonymously`; production JWKS still returns zero asymmetric keys | Client implementation complete; migrate/rotate ES256, and independently deploy the public Turnstile site key before enabling its Supabase-held secret |
| DigitalOcean encrypted variables and deployment of reviewed commit | `/version.json`, HTML marker, headers, health, cookie behavior, secret-free bundle, and live E2E | Proven: live E2E passed runtime commit `d5380679dea1a738132a23dc983cb532976b8e24`; production now reports healthy `196f73ab8645b108a23492585d727df5384ac428`, adding tested CAPTCHA/ES256 verification paths without changing the validated audio pipeline |
