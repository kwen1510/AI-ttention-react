...
if (!connectionUrl || !archiveBatch || process.env.CONFIRM_RESET !== "RESET_OPERATIONAL_DATA") {
  console.error("Refusing reset: require SUPABASE_DB_URL, ARCHIVE_BATCH, and CONFIRM_RESET=RESET_OPERATIONAL_DATA.");
  process.exit(1);
}
...
await runPostgresTool(psql, [
  "--set", "ON_ERROR_STOP=1",
  "--set", `archive_batch=${archiveBatch}`,
  "--set", "confirm_reset=RESET_OPERATIONAL_DATA",
  "--file", path.join(root, "server/db/cutover/reset_operational_data.sql")
], env);
console.log(`Operational data reset after verified archive ${archiveBatch}; prompts and teacher access were preserved.`);
